...
这套工作流用于把“任务分析 / 需求整理 / 开发计划”类输出拆成三个稳定角色：

- `Claude Opus 4.6` 负责生成与修订主 Markdown 文档
- `GPT-5.4` 负责 review 当前版本，并输出修改计划
- 主控制器负责编排轮次、保存文件、判断收敛或停机

目标不是让两个模型同时写文档，而是让一个模型负责产出，一个模型负责挑错，再由产出方吸收或反驳意见，直到收敛。
...
- `使用双模型共识工作流分析这个需求`
- `用双模型共识工作流生成开发计划`
- `让 Claude 起草，GPT review，直到一致`

默认情况下，不会自动拦截所有普通分析或计划请求。

## 工作流资源

项目内相关文件：

- Skill: `.cursor/skills/dual-model-consensus/SKILL.md`
- 参考规范: `.cursor/skills/dual-model-consensus/reference.md`
- Prompt 模板:
  - `.cursor/prompts/dual-model-consensus/claude-analysis-planner.md`
  - `.cursor/prompts/dual-model-consensus/gpt-review.md`
  - `.cursor/prompts/dual-model-consensus/claude-revision.md`
  - `.cursor/prompts/dual-model-consensus/disagreement-report.md`
- 项目级 subagent:
...
```mermaid
flowchart TD
    userRequest[UserRequest] --> skillEntry[dualModelConsensusSkill]
...
- `user task`
- `artifact type`，可选 `analysis` 或 `plan`
- `topic slug`
- `max rounds`，默认 `3`
...
原因是 subagent 不继承父对话历史，不能假设它知道前文发生了什么。
...
Cursor 官方文档支持为 subagent 配置独立 `model`，但在某些运行模式下该配置可能不会完全按预期生效。因此需要保留手动回退路径。
...
```text
请使用 dual-model-consensus skill 处理下面这个任务：
...
```text
Use the claude-author subagent for round 1.
Artifact type: plan
Topic slug: api-gateway-rollout
Save target: .cursor/plans/api-gateway-rollout/draft-r1.md
Rendered prompt:
<粘贴 claude-analysis-planner.md 渲染后的完整文本>
```

下一轮 review：

```text
Use the gpt-reviewer subagent for round 1.
Artifact type: plan
Topic slug: api-gateway-rollout
Save target: .cursor/plans/api-gateway-rollout/review-r1.md
Current artifact:
<粘贴 draft-r1.md 正文>
Rendered prompt:
<粘贴 gpt-review.md 渲染后的完整文本>
```
