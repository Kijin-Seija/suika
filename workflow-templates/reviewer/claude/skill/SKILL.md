---
name: reviewer
description: 当用户显式要求使用 reviewer skill 时使用。Claude 先完成主任务，再通过安装后的 launcher 启动外部 Codex reviewer 子进程做结构化 review，并按 review 结果自动迭代修订，直到通过或达到轮次上限。
---
...
- 外部 Codex reviewer 子进程负责 review 当前制品，并返回结构化 JSON
...
- 调用 `.claude/skills/reviewer/bin/reviewer-run.sh` 获取 review
- launcher 必须以 `codex exec -C <project> -s read-only` 在当前主 agent 的工作目录与工作区状态中启动外部 reviewer，不得切换到独立 worktree、临时副本或其他隔离目录
- 默认显式传入模型 `gpt-5.4`；可通过 `REVIEWER_CODEX_REVIEW_MODEL` 覆盖，并保留 `REVIEWER_CODEX_BIN` / 兼容 fallback 的运行时变量约定
...
7. 控制器必须调用 `.claude/skills/reviewer/bin/reviewer-run.sh review ...` 审查当前制品，而不是在对话中模拟 reviewer。
8. launcher 使用当前项目目录执行 `codex exec -C <project> -s read-only ... -m gpt-5.4`；如果设置了 `REVIEWER_CODEX_REVIEW_MODEL`，则改为显式传入该模型。
9. 将 Codex 原样返回保存为 `review-r1.md`。
9. 校验返回是否为符合契约的 JSON：
...
- Codex reviewer 必须在当前项目工作区中运行，直接查看主 agent 已产生但尚未提交的改动
- 不得为 Codex reviewer 创建或切换到独立 worktree、临时 checkout、镜像目录或其他看不到当前改动的隔离环境
