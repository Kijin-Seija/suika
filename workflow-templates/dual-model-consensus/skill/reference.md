...
为节省 token，控制器在不影响判断质量的前提下，应优先传“紧凑上下文包”，而不是把完整历史或完整制品反复注入。

### `REVISION_CONTEXT`（plan 模式）

推荐包含：

- 文档骨架：标题与章节清单
- 受最新 review 影响的章节正文或必要片段
- 最新 review 中尚未解决的 findings 与 requested change
- 对未展示章节的保留约束

只有在最新 review 明确要求整体重写、或局部片段不足以保持一致性时，才回退为传入完整上一版主制品。
...
历史 review 与 response 文件应保留在磁盘中供人工追溯，但不应默认整包注入到分歧报告 prompt。

## 最小化控制器渲染模板

下面给出一组可直接复用的最小模板。目标是让控制器只向 subagent 发送“渲染后的 prompt + 极少量运行说明”，并把真正占 token 的上下文压缩进几个稳定的上下文包。
...
## Keep Unchanged
- 未展示章节默认沿用上一版
- 不要改动既有章节顺序，除非 review 明确要求
```

如果 review 明确要求整体重写，再回退为“完整上一版主制品 + 最新 review”。
...
## File Notes
- `path/to/file1`: <这个文件为什么改>
- `path/to/file2`: <这个文件为什么改>
...
不要把 `draft-r1.md`、`revision-rN.md` 或 `final.md` 的完整 Markdown 原样粘贴到这里；如果需要引用其中信息，必须先抽取为增量 diff、文件说明或必要片段。
...
LATEST_REVIEW_SUMMARY
- verdict: acceptable | revision-required
- blocking:
  - <问题或 none>
- important:
  - <问题或 none>
...
LATEST_CLAUDE_POSITION
- source: response | inferred-from-artifact
- accepted:
  - <已接受项>
- rejected:
  - <拒绝项及理由>
...
- 发送给 GPT 的内容优先是增量 diff + 受影响文件摘要，只有在必要时才补充关键片段
...
- 渲染后的 Claude 修订 prompt 包含 `REVISION_CONTEXT`
- `REVISION_CONTEXT` 优先只包含文档骨架、受影响章节、最新 findings 和保留约束；只有在必要时才回退为完整上一版主制品
...
- `CODE_REVIEW_CONTEXT` 优先包含自上一轮 review 以来的增量 diff、受影响文件摘要和必要片段，而不是当前累积全量 diff
...
- 已达到配置的轮次上限
- 最新主制品和最新 review 都可用
- 渲染后的 disagreement prompt 包含最新制品摘要、最新 review 摘要、最新 Claude 立场摘要和未解决问题列表
- 如果没有发生 Claude 修订轮次，则 `LATEST_CLAUDE_POSITION` 从最新主制品推断，并显式标注为 inferred
