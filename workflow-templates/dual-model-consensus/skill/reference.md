...
如果上下文因 token 限制只能给出带 `...` 的摘录，控制器仍应尽量附上可定位原文的仓库内文件路径；reviewer 收到这类输入后，应先根据这些路径自行补齐原始上下文，再决定是否能继续审阅。只有在原文既未提供、又无法根据给定线索恢复时，才应以“上下文不足”作为 blocking 问题。
...
- 如果摘录中存在 `...` 或省略段落，优先同时提供相关文件路径，便于 reviewer 自行恢复原文
...
如果 `Incremental Diff` 或 `Extra Snippets` 中出现 `...` 或缺失 hunk，必须保留足够的文件路径线索，使 reviewer 可以自行读取原文恢复上下文。
...
- 如果 `CURRENT_ARTIFACT` 只能使用带省略号的摘录，prompt 中仍应保留足以恢复原文的仓库内文件路径或引用线索
...
- 如果增量 diff 或片段使用了省略号，仍保留足以恢复原文的仓库内文件路径，供 reviewer 自行补齐上下文
