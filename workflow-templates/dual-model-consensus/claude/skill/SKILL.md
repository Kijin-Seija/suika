...
- 控制器负责流程调度、收敛判断和文件落盘，但不参与主制品创作或修订
...
- `.claude/plans/<topic-slug>/disagreement-report.md`

只创建本次实际运行所需的文件。

所有运行产物都必须放在项目根目录下的 `.claude/plans/` 中，并使用 `.claude/plans/<topic-slug>/` 这种 topic 子目录。
...
- prompt 模板：`./prompts/claude-analysis-planner.md`
- 修订模板：`./prompts/claude-revision.md`

code 模式使用：

- prompt 模板：`./prompts/claude-code-draft.md`
- 修订模板：`./prompts/claude-code-revision.md`

推荐 subagent：`.claude/agents/claude-author.md`
...
- prompt 模板：`./prompts/gpt-review.md`

code 模式使用：

- prompt 模板：`./prompts/gpt-code-review.md`

推荐 subagent：`.claude/agents/gpt-reviewer.md`
...
- 可以做：prompt 渲染、角色调度、文件保存、哨兵拆分、`git diff` 捕获、格式校验、收敛判断、重试或停止流程
- 不可以做：改写 `plan` 正文、修改代码、补写遗漏章节、替作者落实 reviewer 建议、润色或重组主制品、合并或降级 reviewer finding 后再转述
- 如果角色输出不合规，控制器只能要求同一角色重试，或停止流程并输出失败结果；不能自己补救正文
...
11. 如果当前 GPT review 轮次已经达到上限，则渲染分歧报告 prompt，包含：
    ...
    - `{{TOPIC_SLUG}}`
    - `{{MAX_ROUNDS}}`
    - `{{LATEST_ARTIFACT}} = 最新主制品原文，必要时仅做引用式摘录`
    - `{{LATEST_REVIEW}} = 最新 review 原文，必要时仅做引用式摘录`
    - `{{LATEST_CLAUDE_RESPONSE}} = 最新 Claude 响应原文；如果尚未发生 Claude 修订轮次，则留空并标记 none`
    - `{{UNRESOLVED_ISSUES}} = 从最新 review 中逐条引用的未解决问题，不做语义改写`
12. 调用分歧报告 prompt 生成未收敛总结，并保存为 `disagreement-report.md`。
13. 否则，渲染 Claude 修订 prompt，包含：
    - `{{ROUND}} = 下一轮 review 的轮次号`
    - `{{CURRENT_ARTIFACT}} = 最新主制品原文`
    - `{{LATEST_REVIEW}} = 最新 review 原文`
...
6. 捕获代码变更：若本轮新增了未跟踪文件，先执行 `git add -N <path>` 将其纳入 diff，再执行 `git diff`（包含未暂存变更）获取完整 diff。
...
8. 渲染 GPT code review prompt（使用 `gpt-code-review.md`），包含：
   ...
   - `{{TOPIC_SLUG}}`
   - `{{ROUND}} = 1`
   - `{{CODE_REVIEW_CONTEXT}} = 以原始增量 diff 为主的 review 上下文；包含文件路径列表，以及仅在必要时附带的原始关键代码片段`
...
13. 如果当前 GPT review 轮次已经达到上限，则渲染分歧报告 prompt，传入相应上下文，保存为 `disagreement-report.md`。控制器只能引用最新主制品、最新 review 和最新 response 的原文，不得先改写成新的业务摘要。
14. 否则，渲染 Claude 代码修订 prompt（使用 `claude-code-revision.md`），包含：
    - `{{ROUND}} = 下一轮 review 的轮次号`
    - `{{CODE_REVIEW_CONTEXT}} = 自上一轮 review 以来的原始增量 diff，以及仅在必要时附带的原始代码片段或文件路径列表`
...
- `{{CURRENT_ARTIFACT}}`（Claude 修订时传入最新主制品原文）
- `{{LATEST_REVIEW}}`（Claude 修订时传入最新 review 原文）

### code 模式变量

- `{{USER_TASK}}`、`{{ARTIFACT_TYPE}}`、`{{TOPIC_SLUG}}`、`{{ROUND}}`、`{{MAX_ROUNDS}}`
- `{{CODE_REVIEW_CONTEXT}}`（GPT review 和 Claude 修订时传入，包含原始增量 diff、必要的原始代码片段和文件路径列表；不是 `draft-r1.md` / `revision-rN.md` / `final.md` 的原文转发，也不是控制器撰写的语义摘要）
...
- 立即停止循环
- 使用 `./prompts/disagreement-report.md` 生成 `disagreement-report.md`
...
优先使用 `.claude/agents/` 下的项目级 subagent，让每个角色都能请求其目标模型。

但控制器必须把模型绑定视为 best-effort 的运行时设置。如果 Claude Code 没有按预期使用 subagent 配置的模型：
...
控制器不得用“热修正文”的方式掩盖角色输出问题。统一规则如下：

- 如果 `writer` 输出缺少必需结构、哨兵或逐条回应，控制器只能要求 `writer` 重试该轮
- 如果 `reviewer` 输出缺少必需结构或严重级别字段，控制器只能要求 `reviewer` 重试该轮
- 如果重试后仍不满足契约，控制器必须停止流程，并输出 `disagreement-report.md` 或报告当前轮失败
- 在任何失败路径中，控制器都不得自行补齐主制品、response 或代码修改
