...
1. 首次进入 `debug-steps` 会话时，执行安装后的 launcher，例如 `.claude/skills/debug-steps/bin/debug-session.sh start`。
...
.claude/skills/debug-steps/bin/debug-session.sh start --session login-bug
.claude/skills/debug-steps/bin/debug-session.sh start --host 127.0.0.1 --port 47821
```
