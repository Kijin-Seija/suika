...
1. 首次进入 debug 会话时，执行安装后的 launcher，例如 `.codex/skills/debug/bin/debug-session.sh start` 或 `.claude/skills/debug/bin/debug-session.sh start`。
2. 启动会话后，如果当前还没有本轮问题的新日志，先要求用户复现并重新写日志；不要在没有读取最新日志的情况下直接基于代码猜根因。
...
8. 用户确认问题已解决后，执行安装后的 launcher `cleanup`，删除 log 文件、状态文件和后台服务。
...
2. 先总结日志中已经发生的事实，例如：
   ...
   - 哪个状态值异常
   - 哪段报错或堆栈最关键
3. 再把这些事实映射到代码路径、数据流或状态机。
4. 再确认这些打印信息是否已经足以证明当前判断，而不只是“看起来合理”。
5. 如果事实还不够，优先补日志字段或补复现，而不是继续扩大代码阅读范围，更不能直接开始修复。
...
.codex/skills/debug/bin/debug-session.sh start --session login-bug
.codex/skills/debug/bin/debug-session.sh start --host 127.0.0.1 --port 47821
# Claude Code 宿主对应使用 .claude/skills/debug/bin/debug-session.sh
```
