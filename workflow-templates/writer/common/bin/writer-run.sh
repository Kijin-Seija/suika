...
usage() {
  cat >&2 <<'EOF'
用法:
  writer-run.sh run --task <task> [--writer <claude|codex>] [--topic <slug>] [--max-rounds <n>] [--workdir <dir>]
EOF
}
...
write_revision_handoff() {
  ...
  local latest_writer_json="$8"
  local latest_artifact="$9"

  python3 - "$handoff_path" "$task" "$topic" "$round" "$max_rounds" "$writer_kind" "$latest_review" "$latest_writer_json" "$latest_artifact" <<'PY'
import json
import os
import sys

handoff_path, task, topic, round_no, max_rounds, writer_kind, review_path, writer_json_path, artifact_path = sys.argv[1:10]
...
compose_artifact_markdown() {
  ...
  python3 - "$json_path" "$output_path" "$diff_path" <<'PY'
import json
import sys

json_path, output_path, diff_path = sys.argv[1:4]
...
- 如果仓库很大，优先先完成最小闭环修复，再把残余问题写入 `questions`
EOF
  fi
...
请先阅读最新 handoff 和最新 review，再修改当前工作区代码，并逐条回应所有 issue。
EOF
}
...
run_loop() {
  ...
  mkdir -p "${plans_dir}"
  write_brief "${brief_path}" "${topic}" "${task}" "${max_rounds}" "${baseline}" "${writer_kind}"
  ...
  run_writer_json "${writer_kind}" build_writer_draft_prompt "${latest_writer_json}" "${SCHEMAS_DIR}/claude-draft.schema.json" "${task}" "${topic}" "${round}" "${max_rounds}" "${brief_path}" "${writer_kind}"

  append_untracked_to_index
  git diff --patch --unified=3 > "${plans_dir}/diff-r1.patch"
  compose_artifact_markdown "${latest_writer_json}" "${artifact_path}" "${plans_dir}/diff-r1.patch"

  while true; do
    review_path="${plans_dir}/review-r${round}.json"
    echo "[writer] round ${round}: Codex review"
    run_codex_review_json build_codex_review_prompt "${review_path}" "${SCHEMAS_DIR}/codex-review.schema.json" "${task}" "${topic}" "${round}" "${max_rounds}" "${baseline}" "${artifact_path}" "${latest_review:-none}" "${latest_response:-none}" "${writer_kind}"
    ...
    if [[ "${status}" == "pass" && "${next_action}" == "approve" ]]; then
      write_final_md "${latest_artifact}" "${plans_dir}/final.md" "${topic}" "${review_path}" "${round}" "${writer_kind}"
    ...
    if (( round >= max_rounds )); then
      ...
      run_codex_markdown build_dispute_prompt "${plans_dir}/dispute-report.md" "${task}" "${topic}" "${max_rounds}" "${latest_artifact}" "${latest_review}" "${latest_response}" "${writer_kind}"
    ...
    write_revision_handoff "${handoff_path}" "${task}" "${topic}" "${round}" "${max_rounds}" "${writer_kind}" "${latest_review}" "${latest_writer_json}" "${latest_artifact}"

    echo "[writer] round ${round}: ${writer_kind} revision"
    run_writer_json "${writer_kind}" build_writer_revision_prompt "${response_path}" "${SCHEMAS_DIR}/claude-response.schema.json" "${task}" "${topic}" "${round}" "${max_rounds}" "${handoff_path}" "${latest_artifact}" "${latest_review}" "${latest_response:-none}" "${writer_kind}"
    local missing_issues=""
    if ! missing_issues="$(validate_response_coverage "${latest_review}" "${response_path}" 2>/dev/null)"; then
      fail "Claude response 未覆盖上一轮全部 issue: ${missing_issues}"
    fi

    append_untracked_to_index
    git diff --patch --unified=3 > "${plans_dir}/diff-r${round}.patch"
    compose_artifact_markdown "${response_path}" "${artifact_path}" "${plans_dir}/diff-r${round}.patch"
...
main() {
  ...
  run_loop "${task}" "${topic}" "${max_rounds}" "${writer_kind}"
}
