---
name: writer
description: 当用户显式要求使用 "writer skill"、"writer workflow"、"实现闭环工作流"、"让 Claude Code 开发并由 Codex 审查" 时使用。只用于代码实现任务：Codex 作为控制器和 reviewer，writer 可选 Claude Code CLI 或独立 Codex CLI 子进程，默认最多 5 轮，未通过则输出争议点交由人类裁决。
---
...
该 skill 只适用于代码实现、bug 修复、重构等 `code` 任务，不适用于 plan、分析或纯文档任务。
...
- 所选 writer 只负责实现和根据 review 修订代码
- 如果选择 `writer=codex`，实现必须通过独立的外部 `codex exec` 子进程完成；不要让当前控制器会话自己兼任 writer，否则会破坏角色边界

如果 launcher 失败但工作区已经被 Claude 修改，不要擅自回滚用户代码；先检查 `.codex/plans/<topic-slug>/` 下的已有产物，再向用户说明当前状态。
