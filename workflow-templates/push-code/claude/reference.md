...
- `$CLAUDE_HOME/push-code-monitor/config.json`
- `$CLAUDE_HOME/push-code-monitor/monitor.db`
...
PUSH_CODE_MR_TITLE_PREFIX='[Claude]'
...
PUSH_CODE_MR_MONITOR_DB_PATH='/absolute/path/to/.claude/push-code-monitor/monitor.db'
PUSH_CODE_MR_MONITOR_CONFIG_PATH='/absolute/path/to/.claude/push-code-monitor/config.json'
...
如果启用了全局 MR monitor，`$CLAUDE_HOME/push-code-monitor/config.json` 里默认会包含：
...
`$CLAUDE_HOME/push-code-monitor/monitor.db` 里会保存所有项目已登记的 MR / thread 映射、项目路径、launcher 路径、最近一次巡检指纹，以及最近巡检历史。
...
如果项目配置里的 `PUSH_CODE_MR_MONITOR_ENABLED=true`，且当前环境存在 `CLAUDE_CODE_SESSION_ID`，launcher 会在 `create-mr` 成功后自动把 `thread_id -> mr_id` 连同 `agent_host=claude`、`project_root`、`launcher_path` 一起登记到全局 `monitor.db`。
...
push-code-monitor.cjs registration-upsert --db-path "$CLAUDE_HOME/push-code-monitor/monitor.db" --mr-id 123 --thread-id thread-1 --branch feature/foo --project-root /path/to/project --launcher-path /path/to/project/.claude/skills/push-code/bin/push-code-run.sh
...
push-code-monitor.cjs config-set --config-path "$CLAUDE_HOME/push-code-monitor/config.json" --claude-bin "$(command -v claude)"
push-code-monitor.cjs dashboard-data --config-path "$CLAUDE_HOME/push-code-monitor/config.json" --db-path "$CLAUDE_HOME/push-code-monitor/monitor.db"
```

也可以通过 `PUSH_CODE_MR_MONITOR_CLAUDE_BIN` 临时覆盖 Claude CLI 路径。

其中：

- `start`：后台拉起一个 monitor 服务；它会一边执行 `scan` 循环，一边在本地启动 dashboard，并把 PID / 日志路径 / dashboard 地址写到 `$CLAUDE_HOME/push-code-monitor/service.json`
...
- `notify-thread`：通过 `claude --resume <session-id> --print <message>` 用固定模板通知指定会话
