---
name: push-code
description: 当用户显式调用 /push-code，或要求使用 "push-code skill"、"push code workflow"、"推当前分支并创建 MR"、"跟进 GitLab code review 到可提交状态" 时使用。适用于需要把当前分支推送到 GitLab、直接调用 GitLab REST API 创建合到主分支的 MR、轮询 online review discussions / 普通 MR notes / mergeability 状态，并根据 review 评论继续修复或发起异议回复的场景。
---
...
- 当前不处于 detached HEAD
- `.claude/skills/push-code/config.env` 已存在且 GitLab 配置完整
- `git`、`python3` 可用
...
- `.claude/skills/push-code/bin/push-code-run.sh`
- `.claude/skills/push-code/bin/push_code_webhook.py`
- `.claude/skills/push-code/config.env`
- `.claude/skills/push-code/reference.md`
...
```bash
.claude/skills/push-code/bin/push-code-run.sh preflight
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh push
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh create-mr
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh wait-review --mr-id <mr_id>
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh rebase-target
.claude/skills/push-code/bin/push-code-run.sh push --force-with-lease
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh threads --mr-id <mr_id>
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh comment \
  --mr-id <mr_id> \
  --thread-id <thread_id> \
...
```bash
.claude/skills/push-code/bin/push-code-run.sh resolve-thread \
  --mr-id <mr_id> \
  --thread-id <thread_id>
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh reopen-thread \
  --mr-id <mr_id> \
  --thread-id <thread_id>
```
...
```bash
.claude/skills/push-code/bin/push-code-run.sh note \
  --mr-id <mr_id> \
  --body-file <reply-file>
```
