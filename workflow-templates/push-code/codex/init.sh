...
global_monitor_dir() {
  printf '%s/push-code-monitor' "$(codex_home_dir)"
}
...
project_monitor_dir() {
  ...
  printf '%s/.codex/skills/push-code/monitor' "${project_root}"
}
...
clean_previous_install() {
  local target_project="$1"
  remove_if_exists "${target_project}/.codex/skills/push-code"
}
...
main() {
  ...
  existing_config="${target_project}/.codex/skills/push-code/config.env"
  ...
  mkdir -p \
    "${target_project}/.codex/skills/push-code/bin" \
    "$(global_monitor_bin_dir)"

  remove_if_exists "$(global_monitor_bin_dir)/install-dingtalk-webhook.sh"

  copy_file "${CODEX_DIR}/skill/SKILL.md" "${target_project}/.codex/skills/push-code/SKILL.md"
  copy_file "${COMMON_DIR}/reference.md" "${target_project}/.codex/skills/push-code/reference.md"
  copy_file "${COMMON_DIR}/bin/push-code-run.sh" "${target_project}/.codex/skills/push-code/bin/push-code-run.sh"
  copy_file "${COMMON_DIR}/bin/push_code_webhook.py" "${target_project}/.codex/skills/push-code/bin/push_code_webhook.py"
  copy_file "${COMMON_DIR}/bin/push-code-monitor.cjs" "${target_project}/.codex/skills/push-code/bin/push-code-monitor.cjs"
  copy_file "${COMMON_DIR}/bin/push-code-monitor.cjs" "$(global_monitor_script_path)"

  chmod +x \
    "${target_project}/.codex/skills/push-code/bin/push-code-run.sh" \
    "${target_project}/.codex/skills/push-code/bin/push_code_webhook.py" \
    "${target_project}/.codex/skills/push-code/bin/push-code-monitor.cjs" \
    "$(global_monitor_script_path)"
  ...
  write_config \
    "${target_project}/.codex/skills/push-code/config.env" \
    "${monitor_db_path}" \
    "${monitor_config_path}" \
    "${previous_config_tmp}"
  ...
  echo "已初始化 Codex 版 push-code 工作流:"
  echo "- 目标项目: ${target_project}"
  echo "- skill: .codex/skills/push-code/SKILL.md"
  echo "- config: .codex/skills/push-code/config.env"
  echo "- launcher: .codex/skills/push-code/bin/push-code-run.sh"
  echo "- project monitor bridge: .codex/skills/push-code/bin/push-code-monitor.cjs"
  if [[ "${project_monitor_enabled}" == "true" ]]; then
    echo "- 全局 MR 定时巡检: 已启用"
